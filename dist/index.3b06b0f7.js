...
},{"Lockr":"jixqW"}],"jixqW":[function(require,module,exports) {
...
parcelHelpers.export(exports, "getPrefixedKey", ()=>getPrefixedKey
);
parcelHelpers.export(exports, "hasPrefix", ()=>hasPrefix
);
parcelHelpers.export(exports, "keys", ()=>keys
);
parcelHelpers.export(exports, "rm", ()=>rm
);
parcelHelpers.export(exports, "sadd", ()=>sadd
);
parcelHelpers.export(exports, "set", ()=>set
);
parcelHelpers.export(exports, "setPrefix", ()=>setPrefix
);
parcelHelpers.export(exports, "sismember", ()=>sismember
);
parcelHelpers.export(exports, "smembers", ()=>smembers
);
parcelHelpers.export(exports, "srem", ()=>srem
);
var PREFIX = '';
...
function getPrefixedKey(key, options) {
    if ((options == null ? void 0 : options.noPrefix) === true) return key;
...
function get(key, missing, options) {
    ...
    try {
        ...
        if (localStorage[queryKey]) value = {
            data: localStorage.getItem(queryKey)
        };
        else value = null;
    }
    if (!value) return missing;
    else if (typeof value === 'object' && typeof value.data !== 'undefined') return value.data;
}
function keys() {
    var prefix = getPrefix();
    var keys1 = [];
    var allKeys = Object.keys(localStorage);
    if (!hasPrefix()) return allKeys;
    allKeys.forEach(function(key) {
        if (key.indexOf(prefix) !== -1) keys1.push(key.replace(prefix, ''));
    });
    return keys1;
}
function getAll(includeKeys) {
    var keys$1 = keys();
    if (includeKeys) return keys$1.reduce(function(accum, key) {
        var tempObj = {};
        tempObj[key] = get(key);
        accum.push(tempObj);
        return accum;
...
function flush() {
    if (getPrefix.length > 0) keys().forEach(function(key) {
